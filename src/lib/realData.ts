...
function cmpRow(teams: Record<string, Team>, a: StandingRow, b: StandingRow): number {
  ...
  const sa = teams[a.code]?.s ?? 70,
    sb = teams[b.code]?.s ?? 70;
...
export function buildFromDB(
  ...
  const thirdRanked: StandingRow[] = GROUP_LETTERS.filter((g) => thirdByGroup[g])
  ...
  qualThirds.forEach((x) => {
    thirdGroupToCode[x.g!] = x.code;
  });
