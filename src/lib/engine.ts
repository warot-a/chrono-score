...
const T: Record<string, { n: string; f: string; iso: string; s: number; h?: boolean }> = {
  MEX: { n: 'Mexico', f: '🇲🇽', iso: 'mx', s: 76, h: true },
  RSA: { n: 'South Africa', f: '🇿🇦', iso: 'za', s: 66 },
  KOR: { n: 'South Korea', f: '🇰🇷', iso: 'kr', s: 75 },
  CZE: { n: 'Czechia', f: '🇨🇿', iso: 'cz', s: 75 },
  CAN: { n: 'Canada', f: '🇨🇦', iso: 'ca', s: 74, h: true },
  BIH: { n: 'Bosnia & Herz.', f: '🇧🇦', iso: 'ba', s: 73 },
  QAT: { n: 'Qatar', f: '🇶🇦', iso: 'qa', s: 67 },
  SUI: { n: 'Switzerland', f: '🇨🇭', iso: 'ch', s: 79 },
  BRA: { n: 'Brazil', f: '🇧🇷', iso: 'br', s: 88 },
  MAR: { n: 'Morocco', f: '🇲🇦', iso: 'ma', s: 82 },
  HAI: { n: 'Haiti', f: '🇭🇹', iso: 'ht', s: 60 },
  SCO: { n: 'Scotland', f: '🏴󠁧󠁢󠁳󠁣󠁴󠁿', iso: 'gb-sct', s: 73 },
  USA: { n: 'United States', f: '🇺🇸', iso: 'us', s: 78, h: true },
  PAR: { n: 'Paraguay', f: '🇵🇾', iso: 'py', s: 72 },
  AUS: { n: 'Australia', f: '🇦🇺', iso: 'au', s: 71 },
  TUR: { n: 'Türkiye', f: '🇹🇷', iso: 'tr', s: 77 },
  GER: { n: 'Germany', f: '🇩🇪', iso: 'de', s: 85 },
  CUW: { n: 'Curaçao', f: '🇨🇼', iso: 'cw', s: 59 },
  CIV: { n: 'Ivory Coast', f: '🇨🇮', iso: 'ci', s: 75 },
  ECU: { n: 'Ecuador', f: '🇪🇨', iso: 'ec', s: 77 },
  NED: { n: 'Netherlands', f: '🇳🇱', iso: 'nl', s: 85 },
  JPN: { n: 'Japan', f: '🇯🇵', iso: 'jp', s: 78 },
  SWE: { n: 'Sweden', f: '🇸🇪', iso: 'se', s: 76 },
  TUN: { n: 'Tunisia', f: '🇹🇳', iso: 'tn', s: 69 },
  BEL: { n: 'Belgium', f: '🇧🇪', iso: 'be', s: 83 },
  EGY: { n: 'Egypt', f: '🇪🇬', iso: 'eg', s: 74 },
  IRN: { n: 'Iran', f: '🇮🇷', iso: 'ir', s: 72 },
  NZL: { n: 'New Zealand', f: '🇳🇿', iso: 'nz', s: 60 },
  ESP: { n: 'Spain', f: '🇪🇸', iso: 'es', s: 92 },
  CPV: { n: 'Cape Verde', f: '🇨🇻', iso: 'cv', s: 62 },
  KSA: { n: 'Saudi Arabia', f: '🇸🇦', iso: 'sa', s: 66 },
  URU: { n: 'Uruguay', f: '🇺🇾', iso: 'uy', s: 82 },
  FRA: { n: 'France', f: '🇫🇷', iso: 'fr', s: 90 },
  SEN: { n: 'Senegal', f: '🇸🇳', iso: 'sn', s: 80 },
  IRQ: { n: 'Iraq', f: '🇮🇶', iso: 'iq', s: 65 },
  NOR: { n: 'Norway', f: '🇳🇴', iso: 'no', s: 79 },
  ARG: { n: 'Argentina', f: '🇦🇷', iso: 'ar', s: 91 },
  ALG: { n: 'Algeria', f: '🇩🇿', iso: 'dz', s: 73 },
  AUT: { n: 'Austria', f: '🇦🇹', iso: 'at', s: 76 },
  JOR: { n: 'Jordan', f: '🇯🇴', iso: 'jo', s: 64 },
  POR: { n: 'Portugal', f: '🇵🇹', iso: 'pt', s: 86 },
  COD: { n: 'DR Congo', f: '🇨🇩', iso: 'cd', s: 70 },
  UZB: { n: 'Uzbekistan', f: '🇺🇿', iso: 'uz', s: 67 },
  COL: { n: 'Colombia', f: '🇨🇴', iso: 'co', s: 81 },
  ENG: { n: 'England', f: '🏴󠁧󠁢󠁥󠁮󠁧󠁿', iso: 'gb-eng', s: 88 },
  CRO: { n: 'Croatia', f: '🇭🇷', iso: 'hr', s: 82 },
  GHA: { n: 'Ghana', f: '🇬🇭', iso: 'gh', s: 72 },
  PAN: { n: 'Panama', f: '🇵🇦', iso: 'pa', s: 66 },
};
...
function ts(day: number, hourLocal?: number) {
  return DAY0 + day * DAYMS + (hourLocal || 0) * 3600000;
}
...
export function build(seed: number): Tournament {
  ...
  Object.keys(T).forEach((k) => {
    teams[k] = Object.assign({ code: k }, T[k]);
  });
  ...
  function blankRow(k: string): StandingRow {
    return { code: k, P: 0, W: 0, D: 0, L: 0, GF: 0, GA: 0, GD: 0, Pts: 0 };
  }
  ...
  const thirdRanked: StandingRow[] = GROUP_LETTERS.map((g) => Object.assign({ g }, thirdByGroup[g])).sort((a, b) => {
    if (b.Pts !== a.Pts) return b.Pts - a.Pts;
    if (b.GD !== a.GD) return b.GD - a.GD;
    if (b.GF !== a.GF) return b.GF - a.GF;
    const sa = teams[a.code].s,
  ...
  function matchSlots(allowed: Record<string, string[]>, groups: string[]) {
    ...
    function avail(s: string) {
      return allowed[s].filter((g) => groups.includes(g) && !used[g]);
    }
  ...
  const QF: Record<number, [string, string]> = {
    97: ['M:89', 'M:90'],
    98: ['M:93', 'M:94'],
    99: ['M:91', 'M:92'],
    100: ['M:95', 'M:96'],
  ...
  function simKO(hk: string, ak: string) {
    ...
    let hs = poisson(lh, rand),
      as_ = poisson(la, rand),
      decided = '';
    if (hs === as_) {
      const eg = rand();
      if (eg < 0.28) {
        if (rand() < 0.5 + dh / 120) hs++;
        else as_++;
        decided = 'a.e.t.';
      } else {
        decided = 'pens';
  ...
  function buildRound(
    defs: Record<number, [string, string]>,
    label: string,
    sched: { start?: number; end?: number; day?: number },
  ) {
    const nos = Object.keys(defs)
      .map(Number)
      .sort((a, b) => a - b);
    nos.forEach((no, i) => {
      const [r1, r2] = defs[no];
