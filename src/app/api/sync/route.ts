...
async function handler(req: NextRequest) {
  ...
  const { data: existing } = await db
    .from('matches')
    .select('id, external_id, ko_number')
  ...
  for (const fm of fdMatches) {
    ...
    const venueId  = Object.entries(venueNameToId).find(
      ([k]) => venueKey.includes(k) || k.includes(venueKey)
    )?.[1] ?? null;
...
const TLA_FIXES: Record<string, string> = {
  'BIH': 'BIH', 'KSA': 'KSA', 'KOR': 'KOR', 'CIV': 'CIV',
  'USA': 'USA', 'GER': 'GER', 'FRA': 'FRA', 'ARG': 'ARG',
};
