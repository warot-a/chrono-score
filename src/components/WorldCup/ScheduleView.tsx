...
const ROUND_ABBR: Record<string, string> = {
  ...
  "Semi-final": "SF", "Third place": "3RD", "Final": "FIN"
};
...
export function MatchRow({ tour, m, now, showDate }: { tour: Tournament; m: Match; now: number; showDate?: boolean }) {
  ...
  const decided = v.played && v.decided
    ? <span className="decided">{v.decided === "pens"
      ? (tour.teams[v.penWinner!]?.n + " win on penalties")
      : "after extra time"}</span>
    : null;
  ...
  return (
    <div className={"mrow " + (v.live ? "is-live" : v.played ? "is-ft" : "is-up")}>
      ...
      <div className="mvenue">
        <div className="vrow"><span>{m.cc + " " + m.city}</span>{status}</div>
        <div className="vstad">{m.venue}{decided}</div>
      </div>
    </div>
  );
}
...
export function ScheduleView({ tour, now }: { tour: Tournament; now: number }) {
  ...
  else if (stage !== "all") list = list.filter(m => m.stage === "ko" && ROUND_ABBR[m.round] === stage || (stage === "FIN" && (m.round === "Final" || m.round === "Third place")));
  ...
  list.forEach(m => {
    ...
    if (idx[key] == null) { idx[key] = byDay.length; byDay.push({ key, t: m.t, items: [] }); }
    ...
    <div>
      <div className="sec-head">
        <div>
          ...
          <div className="sub">104 matches across 16 cities · <b style={{ color: "var(--gold)" }}>{played} played</b> so far</div>
        </div>
      </div>
      <div className="filters">
        {stageTabs.map(([k, lbl]) => (
          <button key={k} className={"fbtn" + (stage === k ? " on" : "")} onClick={() => setStage(k)}>{lbl}</button>
        ))}
      </div>
      {stage === "group" ? (
        <div className="filters groups">
          {[["all", "All groups"]].concat(tour.GROUP_LETTERS.map(g => [g, "Group " + g])).map(([g, lbl]) => (
            <button
              ...
              style={grp === g && g !== "all" ? { background: groupColor(g), color: "#1a0b30", borderColor: groupColor(g) } : undefined}
              onClick={() => setGrp(g)}
            >{lbl}</button>
          ))}
        </div>
        ...
        byDay.map(day => {
          ...
          return (
            <section key={day.key} className="dayblock">
              ...
              <div className="mlist">
                {day.items.map(m => <MatchRow key={m.id} tour={tour} m={m} now={now} />)}
              </div>
            </section>
          );
        })
