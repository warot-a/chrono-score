...
function GroupCard({ tour, g, now, qualThirdGroups }: { tour: Tournament; g: string; now: number; qualThirdGroups: string[] }) {
  ...
  return (
    <div className="gcard">
      <div className="gcard-head">
        <span className="gbadge lg" style={{ background: col }}>{g}</span>
      ...
      <table className="gtable">
        <thead>
          <tr>
            ...
            <th className="hide-s">GF</th><th className="hide-s">GA</th>
            <th>GD</th><th className="c-pts">Pts</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={r.code} className={rowClass(i)}>
              <td className="c-pos"><span className="posdot">{i + 1}</span></td>
              <td className="c-team">
              ...
              <td className="hide-s">{r.GF}</td><td className="hide-s">{r.GA}</td>
              <td className="gd">{(r.GD > 0 ? "+" : "") + r.GD}</td>
              <td className="c-pts"><b>{r.Pts}</b></td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="tp-list">
        {arr.map((r, i) => (
          <React.Fragment key={r.g}>
            {i === 8 ? <div className="cutline"><span>qualification cut-off</span></div> : null}
            <div className={"tp-row" + (i < 8 ? " in" : " gone")}>
              <span className="tp-rank">{i + 1}</span>
              <span className="gbadge sm" style={{ background: groupColor(r.g!) }}>{r.g}</span>
              <Flag code={r.code} tour={tour} />
    ...
    <div>
      <div className="sec-head">
        ...
        <div className="legend">
          <span><i className="lg adv" />Advance</span>
          <span><i className="lg q3" />3rd — qualifying</span>
          <span><i className="lg p3" />3rd — contention</span>
          <span><i className="lg out" />Eliminated</span>
        </div>
      </div>
      {!anyPlayed ? <div className="empty">The group stage has not kicked off yet. Move the tournament clock forward to begin.</div> : null}
      <div className="gcard-grid">
        {tour.GROUP_LETTERS.map(g => <GroupCard key={g} tour={tour} g={g} now={now} qualThirdGroups={qualThirdGroups} />)}
      </div>
